...
RUN sed -i.bak 's/listener.http.internal = 127.0.0.1/listener.http.internal = 0.0.0.0/' /etc/riak/riak.conf && \
    ...
    echo "anti_entropy.concurrency_limit = 2" >> /etc/riak/riak.conf && \
    echo "javascript.map_pool_size = 8" >> /etc/riak/riak.conf && \
    echo "javascript.reduce_pool_size = 6" >> /etc/riak/riak.conf && \
    echo "javascript.hook_pool_size = 2" >> /etc/riak/riak.conf
